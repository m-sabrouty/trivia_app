...
def create_app(test_config=None):
    ...
    def retrieve_all_questions():
        ...
        current_categ_ids = [
            s for s in set([q['category'] for q in current_questions])]
        current_category = Category.query.filter(
            Category.id.in_(current_categ_ids)).all()
        ...
        return jsonify({
            'success': True,
                ...
                'current_category': [c['type'] for c in [c.format() for c in current_category]],
    ...
    def delete_question(question_id):
        try:
            ...
            return jsonify({
                'success': True,
                    'message': 'Question '+str(question_id)+'Deleted Successfully',
                    'total_questions': len(Question.query.all())
        ...
        try:
            if quiz_category_id != 0:
                questions = Question.query.filter(Question.category == quiz_category_id).\
                    filter(~Question.id.in_(previous_questions)).order_by(
                    func.random()).all()

            elif quiz_category_id == 0:
                questions = Question.query.filter(
                    ~Question.id.in_(previous_questions)).order_by(func.random()).all()
